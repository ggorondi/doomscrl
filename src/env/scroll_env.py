...
class DoomScrollEnv(gym.Env):
    ...
    def __init__(
        ...
        self._feed_idx: int = 0
        self._step_count: int = 0
        self._video_step: int = 0
        self._activation_history: np.ndarray = np.zeros(self.hist_len, dtype=np.float32)
    ...
    def step(self, action: int) -> tuple[np.ndarray, float, bool, bool, dict]:
        ...
        truncated = self._feed_idx >= len(self._feed) and self._video_step >= video_T
    ...
    def _load_video(self, feed_idx: int) -> None:
        ...
        self._current_entry = self._feed[feed_idx]
        self._video_step = 0

        features = self._current_entry.load_features()
    ...
    def _get_current_prediction(self) -> torch.Tensor:
        ...
        ctx_len = self._cached_preds.shape[0] - self._video_length(
            self._current_entry.load_features() if self._current_entry else {}
        )
